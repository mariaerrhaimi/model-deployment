...
def predict():
    ...
    #
    ll = [X_test_new]
    new_data = pd.DataFrame(ll, columns=['hashtag_1','playCount','authorMeta_video','creator_likes'])
    prediction = model.predict(new_data)[0]
    output = round(prediction, 2)

    return render_template('index.html', prediction_text='Estimated subscribers =  {} '.format( output))
